...
namespace PostgreSQL_Editor.Utilities
{
    public class DgvChangeDetector : IDisposable
    {
        ...
        {
            // Guard: RowIndex kann -1 sein (z.B. Header, oder nicht gebundene Zeile)
            ...
            if (!_snapshot.TryGetValue(e.RowIndex, out var oldValues) || !RowValuesEqual(oldValues, newValues))
            {
                ...
                // Event auslösen (kopiere Referenz, aber prüfe Index in Empfängern)
        ...
        public void AcceptChanges(DataGridViewRow row)
        {
            ...
            // Schutz: Index kann -1 sein wenn Row noch nicht in Rows-Collection ist
            if (idx >= 0 && idx < _dgv.Rows.Count && !row.IsNewRow)
            {
                _snapshot[idx] = GetRowValues(row);
            }
            // Entferne die Index-Markierung (sicherheitsshalber)
